...
public class JotForm {
	...
	private static org.slf4j.Logger logger = LoggerFactory.getLogger(JotForm.class);
            ...
            if(params != null) {
            	...
            	for(String key: keys) {
            		try {
						...
						logger.error("Error while processing executeHttpRequest"+e.getMessage());
					}
            	}
    ...
    public JSONObject editSubmission(long sid, HashMap<String, String> submission ) {
    	...
    	for(String key: keys) {
    		if (key.contains("_") && !key.equals("created_at")) {
    			...
    			logger.debug("submission[" + key + "] : "+submission.get(key));
    			parameters.put("submission[" + key + "]", submission.get(key));
    		}
    	}
    ...
    public JSONObject createFormQuestion(long formID, HashMap<String, String> question ) {
    	...
    	for(String key: keys) {
    		logger.debug("question[" + key + "]"+ question.get(key));
		...
		for(String formKey: formKeys) {
			...
			if(formKey.equals("properties")) {
				...
				for(String formItemKey: formItemKeys) {
					logger.debug("formItemKey : "+formItemKey);
