...
try{
	...
	log.debug("Crm Lead ID inside Page co 2 "+ crmLeadId);
	ses1.setAttribute("crmLeadId", crmLeadId);
}catch(Exception e
		){
	log.error("errror in getting crmLead ID in page co 2",e);
...
totalcurrentMonthsInt = Integer.parseInt(totalcurrentMonths);
		...
		log.error("error occurs at current month process "+e.getMessage());
// TODO: handle exception
	...
	try{
		...
		log.error("error occurs at current date & month process "+e.getMessage());
	}
	...
	try {
		...
		log.error("error occurs while parsing current date & month detailes "+e.getMessage());
	}	
	...
	if((totalcurrentMonths != null && totalcurrentMonths.length()!=0 && !totalcurrentMonths.equals(""))&& totalcurrentMonthsInt<=36){
		...
		try{
			...
			log.error("error occurs while parsing prior current  month detailes "+e.getMessage());
		...
		if(totalpriorcurrentmonths2Int<=36 && (totalpriorcurrentmonths2!=null && !totalpriorcurrentmonths2.equals(""))){
			...
			try {
				...
				log.error("error occurs while parsingnew date & month detailes "+e.getMessage());
			}	
