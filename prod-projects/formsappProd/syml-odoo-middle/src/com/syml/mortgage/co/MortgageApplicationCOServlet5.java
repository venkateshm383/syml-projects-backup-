...
public class MortgageApplicationCOServlet5 extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		try{
			 crmLeadId=(String) ses.getAttribute("crmLeadId");
			log.debug("Crm Lead ID inside Page co 5  "+ crmLeadId);
			ses.setAttribute("crmLeadId", crmLeadId);
		}catch(Exception e
				){
			log.error("errror in getting crmLead ID in page co 5 ",e);
...
*/		
		...
		try{
			...
			applicantId=(String) ses.getAttribute("applicantId2");
				...
				try{
					...
					applicantIDInt = Integer.parseInt(applicantID2);
					 ...
					 dataStoreValue.put("CoApplicant-otherApplicant",otherApplicant);
						...
						if(leadId==0){
						try{
						JSONObject jsonObject=	storeData.getCouchBaseData("Applicant_"+applicantIDone);
						leadId=Integer.parseInt(jsonObject.get("opportunity_id").toString());
			...
			storeData.appendDataInCouchBase("Applicant_"+applicantIDone, dataStoreValue);
				log.debug("CouchBase data created"); 
             			...
             			req.setAttribute("uniid",uniid);
             				...
             				ses.setAttribute("form1uniqueId", uniid);
             	     		//req.setAttribute("applicantId",applicantId);
             	     		req.getRequestDispatcher("MortgageApplication8.jsp?crmLeadId="+crmLeadId).include(req, res);
