...
public class MortgageForm2Servlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		try{
			String crmLeadId=(String) ses1.getAttribute("crmLeadId");
			log.debug("Crm Lead ID inside Page 2 "+ crmLeadId);
			ses1.setAttribute("crmLeadId", crmLeadId);
		}catch(Exception e
				){
			log.error("errror in getting crmLead ID in page 2 ",e);
...
// TODO: handle exception
		log.error("Error occurs while parsing totalcurrent months"+e.getMessage()); 
}
	...
	try {
		...
		log.error("Error occurs while parsing Current date & time"+e.getMessage());
	}	
...
dataStoreValue.put("Applicant-totalcurrentMonths",totalcurrentMonths);
	...
	if((totalcurrentMonths != null && totalcurrentMonths.length()!=0 && !totalcurrentMonths.equals(""))&& totalcurrentMonthsInt<=36){
		...
		try{
			...
			// TODO: handle exception
			log.error("exception in converting totalpriorcurrentmonths to int : "+e.getMessage());
		}
		...
		try{
			...
			log.error("Error occurs while updating prior address"+e.getMessage());
		}
		...
		if(totalpriorcurrentmonths2Int<=36 && (totalpriorcurrentmonths2!=null && !totalpriorcurrentmonths2.equals(""))){
			...
			try {
				...
				log.error("Error occurs while updating date & time"+e.getMessage());
			}	
			...
			log.error("Error occurs while updating prior address3"+e.getMessage());
			}
			...
			try {
				...
				log.error("Error occurs while updating date & time of address3"+e.getMessage());
			}	
	...
	log.error("Error occurs while storing data of applicant in couchbase"+e.getMessage());
