...
log.info("leadin goal "+leadingGoal);
...
log.debug("old unique id is ----"+ uniid);
			...
			log.error("error occurs at parsing applicant Id",e.getMessage());	
			}
		...
		applicantIDInt = Integer.parseInt(applicantID);
				...
				log.error("error occurs at parsing applicant1 Id",e.getMessage());	
			}
			...
			try{
				applicantID2Int = Integer.parseInt(applicantID2);
						...
						log.error("error occurs at parsing applicant2 Id",e.getMessage());			
					}
			 ...
			 int desiredType=0;
				...
				if(lookingfor11.equalsIgnoreCase("Suggest")){
					...
					try{
						...
						log.error("error occurs while looking Mortgationvalue",e.getMessage());	
					}
		...
		try{
		 condition_of_financing_date=new Date(financingdate1);
			...
			log.error("error occurs while processing financialdate",e.getMessage());	
		}
			...
			dataStoreValue.put("Applicant-refiAdditionalAmount", refiAdditionalAmount);
			    ...
			    try{
			    	...
			    	log.error("error occurs while processing financial date",e.getMessage());	
			    }
               ...
               //String in couchBase
				...
				if(currentmortage1.equalsIgnoreCase("Maximum Amount")){
					...
					refiAdditionalAmount =Integer.parseInt(req.getParameter("refiadditionalamount47"));
						...
						log.error("error occurs while processing applicantion addtional amount",e.getMessage());		
					}
					 dataStoreValue.put("Applicant-refiAdditionalAmount", refiAdditionalAmount);
