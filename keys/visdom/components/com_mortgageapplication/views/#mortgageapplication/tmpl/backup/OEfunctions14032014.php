...
//			    echo "msg>>>";
    ...
    function applicant_record($crmid,$W2DrpRelationship,$len){
		echo "inside application recorddddddddddddddddddddddddddddddddddddddd";
	    ...
	    print_r($arrayApplicantRec); 
		    if (!empty($arrayApplicantRec)){
				...
				if($search_id){
					...
					echo "app_rec_idiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii".$app_rec_id;
				}  
		...
		if ($search_id){
		    ...
		    $response_app_id = $this->search('applicant.address',$key_applicant_add);
		    
	    
		    print_r($response_app_id);
		    $search_add_id = $response_app_id[0];
		    echo "search_add_id>>>###".$search_add_id;			
		    $this->write('applicant.address',$search_add_id, $arrayApplicantAddRec);	  			
		}
		else{
			$app_add_rec_id = $this->create('applicant.address', $arrayApplicantAddRec);
		}			
