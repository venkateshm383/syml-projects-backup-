...
public class MortgageApplicationPage8BService {
	...
	private void setSupplementaryIncomeSrcInApplicant(String[] supplementaryIncomeSrc,Opportunity opportunity){
		...
		for (String mixedString : splits) {
			...
			if (paymentSource.equalsIgnoreCase("LivingAllow")){
				...
				income.setApplicantId(opportunity.getApplicants().get(0).getId());
				opportunity.getApplicants().get(0).getIncomes().add(income);	
			}else if (paymentSource.equalsIgnoreCase("VehicleAllow")){
				income = new Income();
	...
	private Boolean setAddtionalIncomeSrcInApplicant(String[] addtionalIncomeSrc,Opportunity opportunity){
		...
		for (String mixedString : splits) {
			...
			if (paymentSource.equalsIgnoreCase("Investments")){
				...
				income.setApplicantId(opportunity.getApplicants().get(0).getId());
				opportunity.getApplicants().get(0).getIncomes().add(income);
			}else if (paymentSource.equalsIgnoreCase("Pension")){
				income = new Income();
	...
	public void deleteIncomeForSuplementary(Opportunity opportunity) throws MortgageApplicationPageServiceException  {
	    Logger.debug("Inside (.) deleteIncome of  MortgageApplicationPageEightService class ");
	    couchBaseService = new CouchBaseService();
	    posService = new PostGresDaoService();
	    try {
			posService.deleteApplicantIncomeForSuplementary(opportunity.getApplicants().get(0));
		   
			List<Income> incomeList =opportunity.getApplicants().get(0).getIncomes();
